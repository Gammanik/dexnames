...
"asset", "paymentaccnt", "message"] и подписаться ключом от аккаунта который выстелвен на продажу. 
paymentaccnt - аккаунт на который нужно будет перечислять деньги если кто-то купит.


Далее покупатель для каждого выстевленного имени может либо:
 - купить его за выставленную цену. Для этого нужно перевести требуемое количество EOS за покупку на адрес данного смарт-контракта 
 - предложить свою "name": "proposebid", "base": "", "fields": [{"name": "account4sale", "type": "name"},{"name": "bidprice","type": "asset"},{"name": "bidder","type": "name"}]


Если покупатель предложил свою ставку, то продавец может на нее либо согласиться, либо нет. Для того, чтобы решить, продавец отправляет:
"name": "decidebid","fields": [{"name": "account4sale","type": "name"},{"name": "accept","type": "bool"}]
